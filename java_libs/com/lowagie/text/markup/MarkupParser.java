    ...
    public static Properties parseFont(String string) {
        ...
        while (string.length() > 0) {
            ...
            if (value.equalsIgnoreCase("bold")) {
                result.put(MarkupTags.CSS_FONTWEIGHT, MarkupTags.CSS_BOLD);
                continue;
            }
            if (value.equalsIgnoreCase("italic")) {
                result.put(MarkupTags.CSS_FONTSTYLE, MarkupTags.CSS_ITALIC);
                continue;
            }
            if (value.equalsIgnoreCase("oblique")) {
                result.put(MarkupTags.CSS_FONTSTYLE, MarkupTags.CSS_OBLIQUE);
                continue;
            }
            float f;
            if ((f = parseLength(value)) > 0) {
                result.put(MarkupTags.CSS_FONTSIZE, String.valueOf(f) + "pt");
                int p = value.indexOf("/");
                if (p > -1 && p < value.length() - 1) {
                    result.put(MarkupTags.CSS_LINEHEIGHT, String.valueOf(value.substring(p + 1)) + "pt");
                }
            }
            if (value.endsWith(",")) {
                value = value.substring(0, value.length() - 1);
                if (FontFactory.contains(value)) {
                    result.put(MarkupTags.CSS_FONTFAMILY, value);
                    return result;
                }
            }
